...
type HomeScreenViewPropsTypes = NativeStackScreenProps<INavigationParamList, 'Home'>

export default function HomeScreenView({ navigation }: HomeScreenViewPropsTypes) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <HStack justifyContent='space-between' py={2} alignItems='center'>
              ...
              >
                <Icon as={Ionicons} name={'calendar'} size='lg' color={`blue.500`} />
              </Box>
              <VStack flex={1} ml={4}>
              ...
              <Text color={item.status === 'Pending' ? 'orange.400' : 'green.400'}>
                {item.status}
              </Text>
            </HStack>
          )}
